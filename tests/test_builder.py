...
@pytest.fixture
def sample_docs():
    return [Document(page_content="GPT-4o is an OpenAI multimodal model"),
...
@pytest.fixture
def builder():
    return RetrieverBuilder(embeddings=FakeEmbeddings(),
                            reranker=FakeReranker())

@pytest.mark.asyncio
async def test_build_hybrid_retriever(builder,sample_docs):
    retriever = await builder.build_hybrid_retriever(sample_docs,persist=False)
...
async def test_rerank_returns_sorted_docs(builder, sample_docs):
    ranked = await builder.rerank("GPT4o",sample_docs)
...
async def test_empty_documents(builder):
    retriever = await builder.build_hybrid_retriever([],persist=False)
    results = retriever.invoke('test')
    assert results == []

@pytest.mark.asyncio
async def test_rerank_empty_input(builder):
    ranked = await builder.rerank("test", [])
    assert ranked == []